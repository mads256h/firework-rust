...
impl Sparkle {
    ...
    pub fn update_and_draw(
        ...
        let new_position = old_position + (self.direction * delta_time * 100.0 * life_time_01)
            - Vector2D::new(0.0, gravity_factor * delta_time);
        ...
        let color = Color::rgbf(
            self.color.r * life_time_01,
            self.color.g * life_time_01,
            self.color.b * life_time_01,
        );
        ...
        canvas.stroke_path(
            &mut path,
            Paint::color(color).with_line_width(life_time_01 * 1.2),
        );
